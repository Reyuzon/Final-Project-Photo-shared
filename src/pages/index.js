...
export default function HomePage({ user }) {
  ...
  const [likePosts, setLikePosts] = useState(
    getCookie("likePosts") ? JSON.parse(getCookie("likePosts")) : []
  );

  const [savePosts, setSavePosts] = useState(
    getCookie("savePosts") ? JSON.parse(getCookie("savePosts")) : []
  );

  const handleLikePost = (id) => {
    setLikePosts([...likePosts, id]);
    setCookie("likePosts", JSON.stringify([...likePosts, id]));
  };

  const handleUnlikePost = (id) => {
    setLikePosts(likePosts.filter((post) => post !== id));
    setCookie(
      "likePosts",
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      {isLoading
        ...
        : data.pages.map((page) =>
            page.data.data.posts.map((post, i) => (
              <div className="relative border-b w-[90%] md:w-[60%] pb-5">
                <div className="flex items-center gap-3 p-4 right-0 bg-white">
                  <Img
                    src={
                      post.user
                ...
                <Img
                  className="w-auto rounded-xl mb-4 aspect-square mx-auto"
                  key={i}
                  src={post.imageUrl}
                  fallback={
                    <div class="flex items-center justify-center w-full aspect-square rounded-3xl mb-4 bg-gray-300 dark:bg-gray-700">
                      <svg
                        class="w-full h-[80%] text-gray-200 dark:text-gray-600"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="currentColor"
                        viewBox="0 0 16 20">
      ...
      {isFetchingNextPage && (
        <div className="columns-[9rem] md:columns-[13rem] lg:columns-3xs">
